...
pub fn create_engine() -> TemplateEngine {
  ...
  handlebars.register_templates_directory("templates/", options).unwrap();

  Arc::new(handlebars)
}

pub fn render_response(template_engine: &TemplateEngine, template_name: &str, data: &impl serde::Serialize) -> Response {
